...
## Aggiornamento Finale (2026-05-05 - responsive mobile system)

- Introdotta protezione mobile contro overflow orizzontali:
  - `html`, `body` e `#root` vincolati a `max-width: 100%` con `overflow-x: hidden`;
  - media e controlli form protetti da dimensioni minime non elastiche;
  - testi lunghi critici, in particolare IBAN, resi wrappabili.
- Modali runtime consolidati con:
  - padding safe-area;
  - `max-height` basata su `100dvh`;
  - scroll interno quando il contenuto supera l'altezza disponibile.
- Componenti visuali mobile raffinati:
  - CTA Home con larghezza elastica `max-w-xs`;
  - KPI Admin compressi in modo controllato su 320px;
  - righe decorative data/citta in Intro e Pass rese elastiche.
- Verifica browser eseguita con Chrome headless su viewport CSS 320/360/390: nessun overflow orizzontale rilevato sulle route principali e sul modale IBAN.
